...
#nomeBase = 'sce/unbalanced/colecao_dourada_3_class_unbalanced.csv'
def executar(experimento,nome_Base,acento):
    ...
    def find_features(document):
        ...
        return features
    '''
    def idf(todas_as_palavras,palavra):
        todas = set(todas_as_palavras)
        i = 0
        l = len(todas_as_palavras)
        count = 0
        # print(palavra)
        while i<l:
            if(todas_as_palavras[i]==palavra):
                count +=1
            i+=1
        return count

    def tf_idf(todas_as_palavras):
        features = []
        i = 0
        l = len(todas_as_palavras)
        palavras = []
        count = 0
        while i<l:
            if(todas_as_palavras[i] not in palavras):
                palavras.append(todas_as_palavras[i])
                f = idf(todas_as_palavras,todas_as_palavras[i])/l
                fi = -((math.log(f,l))/(f))
                features.append(tuple([todas_as_palavras[i],fi]))
            i+=1
        return features
    ...
    documents = [[wordbigram(w[0]),w[1]] for w in documents if (w[1]=='1' or w[1]=='2')]
    # print([w for w in documents ])
    word_features = wordFeature(documents)
    
    freq = tf_idf(word_features)
    ...
    def removerpalavras(todas_palavras,document):
        ...
        linha = {}
        i = 0
        l = len(todas_palavras)
        while i<l:
            if(todas_palavras[i][0] in document):
                linha[str(i)] = todas_palavras[i][1]
            else:
                linha[str(i)] = 0
            i+=1
        return linha
    # documents = [[removerpalavras(freq,w[0]),w[1]] for w in documents]
    # featuresets = [(removerpalavras(freq,w[0]),w[1])for w in documents]
...
print('Stopwords')        
print('\t\t\t NB \t\t RL \t\t SVC')
executar('experimento3/','sce/unbalanced/colecao_dourada_2_class_unbalanced.csv',False)
executar('experimento3/','sce/unbalanced/colecao_dourada_3_class_unbalanced.csv',False)
executar('experimento3/','sce/balanced/colecao_dourada_2_class_balanced.csv',False)
executar('experimento3/','sce/balanced/colecao_dourada_3_class_balanced.csv',False)
executar('experimento4/','sce/unbalanced/colecao_dourada_2_class_unbalanced.csv',True)
executar('experimento4/','sce/unbalanced/colecao_dourada_3_class_unbalanced.csv',True)
executar('experimento4/','sce/balanced/colecao_dourada_2_class_balanced.csv',True)
executar('experimento4/','sce/balanced/colecao_dourada_3_class_balanced.csv',True)
